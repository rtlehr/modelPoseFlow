...
export async function downloadPosePack(id: number): Promise<{success: boolean; message: string; poses: Pose[]}> {
  const response = await apiRequest('POST', `/api/pose-packs/${id}/download`);
  return response.json();
}