  ...
  useEffect(() => {
    ...
    if (difficultyFilter !== null) {
      filtered = filtered.filter(
        (pose) => pose.difficultyLevel === difficultyFilter,
      );
    }
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((pose) => {
        ...
        if (pose.keywords && Array.isArray(pose.keywords)) {
          if (
            pose.keywords.some((keyword: string) =>
  ...
  const generateKeywordsForPose = async (pose: Pose): Promise<Pose> => {
    ...
    try {
      ...
      while (retryCount <= MAX_RETRIES) {
        try {
          console.log(
            `Attempting to generate keywords for pose ID: ${pose.id} (Attempt ${retryCount + 1}/${MAX_RETRIES + 1})`,
          );
          const keywordsResponse = await apiRequest(
            "POST",
            `/api/poses/${pose.id}/generate-keywords`,
          );
          ...
          if (keywordsResponse.ok) {
            ...
            console.error(
              `Failed to generate keywords (Attempt ${retryCount + 1}/${MAX_RETRIES + 1}):`,
              errorText,
            );
            ...
            if (
              keywordsResponse.status === 404 ||
              keywordsResponse.status === 400
            ) {
              break;
            }
          }
          ...
          console.error(
            `Error during keyword generation attempt ${retryCount + 1}/${MAX_RETRIES + 1}:`,
            attemptError,
          );
      ...
      console.error(
        `Failed to generate keywords after ${MAX_RETRIES + 1} attempts`,
      );
  ...
  const handleUploadPose = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Starting keyword generation for newly uploaded pose:",
          uploadedPose.id,
        );
        ...
        console.log(
          "Fallback: Setting selected pose without keywords:",
          uploadedPose,
        );
        ...
        toast({
          ...
          description:
            "The pose was uploaded but keyword generation failed. You can generate keywords manually.",
        });
      }
    } catch (error: any) {
...
`;
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-lg ${isMobile ? "p-4" : "p-6"} max-w-4xl mx-auto`}
    >
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <h1
          className={`${isMobile ? "text-xl" : "text-2xl"} font-bold text-gray-800 flex-1 text-center`}
        >
          Pose Library
        </h1>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium">
                          {pose.keywords && pose.keywords.length > 0
                            ? pose.keywords[0].charAt(0).toUpperCase() +
                              pose.keywords[0].slice(1)
                            : "No keyword"}
                        </span>
                        <span className="text-xs text-gray-500">
                          ID: {pose.id}
                        ...
                        {pose.difficultyLevel && (
                          <div
                            className={`text-xs px-1.5 py-0.5 rounded-full border ${
                              pose.difficultyLevel === 1
                                ? "text-green-500 border-green-500"
                                : pose.difficultyLevel === 2
            ...
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex items-center gap-2 bg-blue-50 p-3 rounded-md border border-blue-100">
                ...
                <p className="text-xs text-blue-700">
                  After upload, AI will analyze the image and automatically
                  generate keywords for enhanced pose matching.
                </p>
              </div>
            </div>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this pose? This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="flex space-x-2 justify-end">
            ...
            <Button variant="destructive" onClick={handleDeletePose}>
              Delete Permanently
            </Button>
          </DialogFooter>
        </DialogContent>
