  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-gradient-to-b from-indigo-900 to-black overflow-auto">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1588345921523-c2dcdb7f1dcd?ixlib=rb-1.2.1&auto=format&fit=crop&w=1950&q=80')",
        }}
      ></div>
      ...
      <div className="z-10 flex flex-col items-center justify-center min-h-screen px-6 py-16">
        <div className="w-full max-w-md">
          <h1
            className={`font-bold text-white text-center mb-8 ${isMobile ? "text-3xl" : "text-4xl"}`}
          >
            ...
            {menuItems.map((item, index) => (
              ...
              >
                <div className="flex items-center">
                  <div
                    className={`p-2 rounded-full mr-3 ${item.primary ? "bg-white/20" : "bg-white/10"}`}
                  >
                    {item.icon}
                  </div>
                  <div>
                    ...
                    <div
                      className={`text-xs ${item.primary ? "text-white/80" : "text-white/60"}`}
                    >
                      {item.description}
                    </div>
                  </div>
